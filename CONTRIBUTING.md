...
Runtime support for the plugin follows the Node.js and Homebridge versions declared in `package.json`. The development toolchain currently requires recent patch releases within the supported Node.js majors, so using the latest available Node.js 22.x/24.x/26.x release is recommended when contributing.
