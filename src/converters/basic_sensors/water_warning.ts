...
export class WaterWarningSensorHandler extends BasicSensorHandler {
  public static readonly exposesName: string = 'water_warning';
  ...
  constructor(expose: ExposesEntryWithProperty, allExposes: ExposesEntryWithBinaryProperty[], accessory: BasicAccessory) {
    super(
      ...
      expose,
      allExposes,
      WaterWarningSensorHandler.generateIdentifier,
      (n, t) => new hap.Service.ContactSensor(n, t),
      'water_warning'
    );
    accessory.log.debug(`Configuring WaterWarningSensor for ${this.serviceName}`);
  ...
  public static generateIdentifier(endpoint: string | undefined) {
    let identifier = 'water_warning_' + hap.Service.ContactSensor.UUID;
