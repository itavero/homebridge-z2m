...
// Uses prepareDefinition to process extensions and generate exposes arrays
function loadAllDefinitions(): Definition[] {
  const devicesDir = path.join(path.dirname(require.resolve('zigbee-herdsman-converters')), 'devices');
  const files = fs.readdirSync(devicesDir).filter((f) => f.endsWith('.js'));
  ...
  for (const file of files) {
    try {
      ...
      if (mod.definitions && Array.isArray(mod.definitions)) {
        ...
        const prepared = mod.definitions.map((def: Definition) => {
          try {
            return prepareDefinition(def);
          } catch (e) {
            console.log(`Error preparing definition for ${def.model}: ${e}`);
            return def;
          }
        });
        allDefs = allDefs.concat(prepared);
      }
    } catch (e) {
      console.log(`Error loading device file ${file}: ${e}`);
    }
  }
  return allDefs;
}
...
`;
  ...
  if (device.whiteLabel) {
    ...
    devicePage += `\n## White-label models\n${device.whiteLabel
      .map((d) => `* [${d.vendor ?? device.vendor}](../index.md#${normalizeNameForAnchor(d.vendor ?? device.vendor)}) ${d.model}`)
      .join('\n')}\n`;
  }
...
//////////
// DummyDevice object for calling exposes functions
const dummyDevice = { isDummyDevice: true as const };
  ...
  ...definitions
    ...
    .map((d) => {
      ...
      if (typeof d.exposes === 'function') {
        ...
        d.exposes = d.exposes(dummyDevice, {});
      }
